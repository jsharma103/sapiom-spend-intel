...
3. **Post-hold execution failure → the full hold is CAPTURED, not released.** `dryrun/hold_linearity_extension.md`'s 128k-token call errored with a `502 Bad Gateway` *after* its $0.076803 hold was authorized — and that hold was billed as the final settled cost in full, confirmed by an exact $0.076803 balance drop (not a transient dip that reverted). `dryrun/failure_capture_n3.md` replicated this 3 more independent times (same 128k rung, 3 fresh calls) — each captured another exact $0.076803. **CONFIRMED N=4/4 (100%, zero variance):** every observed post-hold execution failure captured the full hold, none floor-settled, none released. This is R1's original scenario (a hold that survives a mid-EXECUTION death) and it does **not** recover — the opposite of "clean reject" behavior. The capture *mechanic* is now deterministic in our data; how often a call fails post-hold in ordinary (unforced) traffic remains unmeasured — all 4 observations were deliberately provoked via 128k over-requests, not sampled from normal calls.
...
16. [REPRODUCED 4/4 (100%, zero variance) — the capture mechanic is now deterministic in our data, 2026-07-04] "Two failure-lifecycle findings, combined, produce a worst case: (a) holds are priced off the *requested* `max_tokens`, confirmed linear to 64k tokens, even when that's 4x past `gpt-4o-mini`'s real ~16k completion ceiling — the model silently accepts and gets held against a number it can never produce; (b) when a call then fails mid-flight, the FULL hold is captured as the final settled cost, not refunded or superseded — now confirmed 4/4 (100%, zero variance, mean $0.076803) across the original 128k gateway-502 plus 3 independent replications (`dryrun/failure_capture_n3.md`). So the worst case is: a caller over-requests `max_tokens`, the call fails for a transient/unrelated reason, and they're billed the maximum possible amount for zero output. **Caveat: the capture-on-failure MECHANIC is measured (4/4, deterministic); the RATE at which calls fail post-hold in live traffic is not** — all 4 observations were deliberately forced via 128k over-requests, not sampled from ordinary calls, so no fleet-wide dollar figure should be quoted from this without labeling that rate assumed. For Jordi: is capture-on-failure the intended semantics of the x402 `\"upto\"` scheme, or should a gateway-level failure (as opposed to a clean model rejection) release the hold? For Zerbib: is 'recoverable' meant to cover this case — a hold that survives a mid-execution death — or only clean-reject paths?" — earned by: `dryrun/hold_linearity_extension.md` (128k rung), `dryrun/failure_capture_n3.md` (3x replication, N=4/4 total), `findings.md` §9/§10, BACKLOG R1 (now answered).
...
17. [CONFIRMED n=5, 2026-07-07 — service_name attribution gap has TWO distinct causes] "Every Firecrawl scrape call lands in the ledger as `service_name: 'unknown'` — 5 of 5 across two independent sessions (the original sweep + 4 survey calls, txn ids in `dryrun/service_hold_survey.md`) — so a whole service family is invisible in per-service revenue attribution; at $0.009/call it's also the priciest flat service we measured. Separately, pre-gateway failures (e.g. our client-side DNS death) also land 'unknown' — structurally, because the platform never learns which service was intended. Two different root causes wearing one label: is the Firecrawl 'unknown' a missing integration label mapping (one-line fix), and would you want unattributed-revenue surfaced as a standing ledger check so the next unlabeled integration gets caught the day it ships?" — earned by: `dryrun/service_hold_survey.md` (n=4 scrape evidence + raw txn ids), attribution-completeness tile (2/81 originally, now n=5 scrape-class), loss_rate.md (the pre-gateway 'unknown' failure). Frame as gift: diagnosis with txn ids + the DQ-contract remedy, not a gotcha.
