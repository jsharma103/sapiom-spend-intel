...
**Case B — failure AFTER the hold is placed → the FULL hold is RETAINED/FROZEN, not released.** `dryrun/hold_linearity_extension.md`'s 128k-token rung of the hold-linearity ladder hit a `502 Bad Gateway` from OpenRouter mid-flight, after the $0.076803 hold (`128,000 × $0.0006/1k`) was already authorized. That transaction's outcome was `error`, yet its cost row stayed `isActive: true` for the entire hold — no superseding capture row ever brought it down to the ~$0.0001 floor the way every successful call in the same experiment did. Confirmed directly against the account balance: `availableBalance` dropped by exactly $0.076803 across that step, matching the hold exactly (not a transient dip that later reverted) — **but `totalBalance` never moved** (pre-run $4.721528 → $4.721228 after the run's three successful floor-settles → still $4.721228 after the failed 128k call, per `hold_linearity_extension.md`'s balance table). Result: **the hold is retained/frozen — unavailable to the customer — which is not the same claim as a completed charge against `totalBalance`, and it is not released either.**

**CONFIRMED, N=4/4 (2026-07-04).** `dryrun/failure_capture_n3.md` replicated this 3 more times: 3 fresh, independent `max_tokens=128000` calls, each hit a `502 Bad Gateway`, and each showed the full $0.076803 hold RETAINED/FROZEN (not released) as its active cost — one cost row apiece, `isEstimate:false`, `isActive:true`, `supersededAt:null`, no floor-settle, no release on any; `availableBalance` dropped by the hold amount each time (per `failure_capture_n3_result.json`'s `balanceBefore`/`balanceAfter`) while `totalBalance` did not move. New transaction ids: `c2c2ef60-2a28-4abf-8e61-1bfcae805bd7`, `0d248074-87fd-4f23-845f-c1c92e83fdf4`, `2f8bec77-71f1-4f06-8f3e-9dcd562a1ba9`. Combined with the original: **4 of 4 (100%) errored-post-hold over-request calls had the FULL inflated hold retained/frozen, mean $0.076803, zero variance across all four observations** — mechanically `max_tokens × $0.0006/1k`, no usage-based adjustment. `dryrun/refund_watch.js` is still polling this transaction for a delayed release (last checked: `STILL-CAPTURED` — see `dryrun/refund_watch.log`; the script's own naming predates this wording correction).

**Honest two-part framing — these are not the same claim, do not conflate them:**
1. **Deterministic retention mechanic (MEASURED, 4/4).** IF a call fails after the hold is placed, the full inflated hold is retained/frozen, not released — guaranteed in our data (100%, zero variance, N=4). `availableBalance` drops by the hold amount every time while `totalBalance` never moves, so this is confirmed as a frozen/unavailable balance, not confirmed as a completed charge. Combined with over-request pricing (§10 below — holds are priced on requested `max_tokens`, even past the model's real ~16k output ceiling), the worst case is: `max_tokens`-priced dollars frozen for zero output.
...
**The combined story:** whether a failure freezes your capital is a function of lifecycle position, not failure type. Pre-hold failures are free (2/2 observed) — nothing is ever held. Post-hold failures have the entire hold retained/frozen, every time observed (4/4, 100%, zero variance) — available balance drops, but total balance evidence stops short of proving a completed charge. The retention *mechanism* is proven; only its frequency in live, unforced traffic is unknown.
