...
async function main() {
  console.log(`[gated] Firing LLM call model=${MODEL} max_tokens=${MAX_TOKENS} prompt="${PROMPT}" agent=${AGENT_NAME} (rule active: $0.001/day limit)`);
  ...
  try {
    ...
    console.error('[gated] call threw:', errorMsg);
  } finally {
    clearTimeout(timeout);
  }
  console.log(`[gated] HTTP status: ${httpStatus}`);
  console.log(`[gated] body: ${JSON.stringify(body)}`);

  console.log(`[gated] waiting ${SETTLE_WAIT_MS}ms for settlement...`);
  await sleep(SETTLE_WAIT_MS);

  console.log('[gated] fetching /v1/transactions...');
  ...
  console.log(`[gated] newest txn id for agent: ${txn ? pick(txn, ['id']) : 'NONE'}`);
  console.log(`[gated] newest txn outcome: ${txn ? pick(txn, ['outcome']) : 'n/a'}`);
  console.log(`[gated] hold: ${hold}  settle: ${settle}`);

  // Also fetch the rule to see if it reflects any denial state / usage.
  let ruleAfter = null;
  try {
    ruleAfter = await gov('/v1/spending-rules/3f3c05dd-11a5-468b-90dc-25cf6ff12f54?include=parameters,conditions,agents');
  } catch (err) {
    console.error('[gated] failed to fetch rule state:', err?.message || err);
  }
  ...
  await writeFile(new URL('./hvs_gated_result.json', import.meta.url), JSON.stringify(result, null, 2));
  console.log('[gated] wrote hvs_gated_result.json');
}
